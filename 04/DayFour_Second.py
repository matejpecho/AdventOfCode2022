...
for input in myinput_file:
    ...
    middle = (int(first_section[1]) - int(second_section[0]))
    edge = (int(second_section[1]) - int(first_section[0]))

    if( (middle >=0 and edge >=0) or (left <= 0 and right >= 0) or (left >=0 and right <= 0)):
        count += 1
